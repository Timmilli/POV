...
uint16_t draw_clock(uint32_t angle, clock_values_t *cv) {
  ...
  datastreak |= draw_fork(angle, (540 - clock_get_seconds(cv) * 6) % 360, 13);
  datastreak |= draw_fork(angle, (540 - clock_get_minutes(cv) * 6) % 360, 10);
  datastreak |= draw_fork(
      angle,
      (540 - (clock_get_hours(cv) % 12) * 30 - clock_get_minutes(cv) / 2) % 360,
      6);
  return datastreak;
}
